...
GET /api/mapdetail           river, lake and terrain geometry, loaded after paint
...
`src/Support/StructuredData.php` emits JSON-LD describing what is genuinely on
each page — never more, since marking up absent content is a manual-action risk
rather than a shortcut:

| Page | Schema |
|---|---|
| Home | `WebSite` with a `SearchAction` |
| Country | `Country` with `GeoCoordinates`, `area`, its capital `City`, and the ten facts as an `ItemList` |
| Section indexes | `CollectionPage` wrapping an `ItemList` |
| Everything below the root | `BreadcrumbList`, matching the visible breadcrumb trail |

### Social preview image

`public/assets/og-cover.png` is a real 1200×630 card, generated by PHP's GD from
the same Robinson projection the site uses, so it always shows the actual atlas:
...
150 ms latency), which is roughly what Google's field data models:

| Page | TTFB | FCP | LCP | CLS |
|---|---|---|---|---|
| Home | 6 ms | 1.04 s | 1.04 s | 0.001 |
| Country | 5 ms | 1.02 s | 1.02 s | 0.001 |
| Countries index | 4 ms | 1.06 s | 1.06 s | 0.001 |

All comfortably inside Google's "good" thresholds (LCP < 2.5 s, CLS < 0.1).

Map pages are still large in absolute terms — around 520 KB of HTML, 177 KB
gzipped — because the country outlines are inline SVG. That is the cost of a
map that renders without a tile server. Rivers, lakes and terrain, another
~260 KB, are **not** inline: they load from `/api/mapdetail` after first paint,
which is what keeps LCP near a second. None of that geometry is indexable text,
so deferring it costs nothing in search terms.
...
src/
  ...
  Support/               Projection, Format, Timezones, Seo, Site, StructuredData
...
tools/
  ...
  build_og_image.php     renders the 1200x630 social card with GD
